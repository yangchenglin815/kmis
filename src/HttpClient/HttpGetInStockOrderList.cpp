...
HttpGetInStockOrderList::HttpGetInStockOrderList(QString sToken, QString sCorpID,QString sUUID, QString sStartDate, QString sEndDate, QString searchPatter
                                                         , int nLimit, int nOffset)
{
    ...
    QString systemParam = CommonFun::generateSystemParams(c_sGetInStockOrderListAction);
...
void HttpGetInStockOrderList::processResponse(QString strResult)
{
       qDebug()<<"instock"<<strResult;
    ...
    QList<InStockOrderInfo> orderInfoList;
    if (sc.property("data").isArray())
    {
        InStockOrderInfo orderInfo;
    ...
    HttpSignal::instance()->sigOnGetInStockOrderList(nCode, sMsg, orderInfoList);
    this->deleteLater();
}

void HttpGetInStockOrderList::processError()
{
    QList<InStockOrderInfo> orderInfoList;
    HttpSignal::instance()->sigOnGetInStockOrderList(e_connectServerError, c_sNetError, orderInfoList);
    this->deleteLater();
}
