...
enum ReportLossCategoryItemType
{
    e_reportLossCategoryInfo = Qt::UserRole + 1,
};

ReportLossCategoryDelegate::ReportLossCategoryDelegate(QObject *parent)
    : QStyledItemDelegate(parent)
{

}

void ReportLossCategoryDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    painter->save();

    QVariant vItemInfo = index.data(e_reportLossCategoryInfo);
...
void ReportLossCategoryDelegate::paintContent(QPainter *painter, const QStyleOptionViewItem &option, CategoryInfo &categoryInfo) const
{
...
ReportLossCategoryListView::ReportLossCategoryListView(QWidget *parent)
    : QListView(parent)
...
void ReportLossCategoryListView::appendItem(const CategoryInfo &categoryInfo)
{
    ...
    pItem->setData(variant, e_reportLossCategoryInfo);
...
void ReportLossCategoryListView::clearItems()
{
...
void ReportLossCategoryListView::clickSelectKeyId()
{
    ...
    if (!m_sSelectedKeyId.isEmpty())
    {
        ...
        this->setCurrentIndex(pItem->index());
        QVariant v_itemInfo = pItem->data(e_reportLossCategoryInfo);
...
void ReportLossCategoryListView::setWidth(int width)
{
    m_nItemWidth = width;
}

void ReportLossCategoryListView::setHeight(int height)
{
    m_nItemHeight = height;
}

void ReportLossCategoryListView::mousePressEvent(QMouseEvent *event)
{
    QListView::mousePressEvent(event);
    //获取选中状态
    QModelIndex index = indexAt(event->pos());
    CategoryInfo categoryInfo;
    QVariant v_itemInfo = index.data(e_reportLossCategoryInfo);
    if (v_itemInfo.canConvert<CategoryInfo>())//可以转换
    {
...
void ReportLossCategoryListView::init()
{
    ...
    setItemDelegateForColumn(0, new ReportLossCategoryDelegate(this));//设置图元代理forColumn
