...
enum AdjustPriceGoodsItemType
{
    e_adjustPriceGoodsInfo = Qt::UserRole + 1,
};

AdjustPriceGoodsDelegate::AdjustPriceGoodsDelegate(QObject *parent)
    : QStyledItemDelegate(parent)
{

}

void AdjustPriceGoodsDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    painter->save();

    QVariant vItemInfo = index.data(e_adjustPriceGoodsInfo);
...
void AdjustPriceGoodsDelegate::paintContent(QPainter *painter, const QStyleOptionViewItem &option, GoodsInfo &goodsInfo) const
{
...
AdjustPriceGoodsListView::AdjustPriceGoodsListView(QWidget *parent)
    : QListView(parent)
...
void AdjustPriceGoodsListView::appendItem(const GoodsInfo &goodsInfo)
{
    ...
    pItem->setData(variant, e_adjustPriceGoodsInfo);
}

void AdjustPriceGoodsListView::clearItems()
{
    m_pStandardItemModel->clear();
    m_ItemKeyIdMap.clear();
}

void AdjustPriceGoodsListView::setWidth(int width)
{
    m_nItemWidth = width;
}

void AdjustPriceGoodsListView::setHeight(int height)
{
    m_nItemHeight = height;
}

void AdjustPriceGoodsListView::mousePressEvent(QMouseEvent *event)
{
    QListView::mousePressEvent(event);
    //获取选中状态
    QModelIndex index = indexAt(event->pos());
    GoodsInfo goodsInfo;
    QVariant v_itemInfo = index.data(e_adjustPriceGoodsInfo);
...
void AdjustPriceGoodsListView::init()
{
    ...
    setItemDelegateForColumn(0, new AdjustPriceGoodsDelegate(this));//设置图元代理forColumn
